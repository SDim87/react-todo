...
const RevenueForecastPanel = ({widgetData, currentArm}) => {
  ...
  useEffect(() => {
    if (widgetData.widget_revenue_forecast) {
      ...
      if ((curWidget && curWidget.hours_payment) || (curWidget && curWidget.services_payment)) {
        const newQuantity = []
        newQuantity.push({name: revenueForecastPanelLabel('hours_payment'), value: curWidget.hours_payment})
        newQuantity.push({name: revenueForecastPanelLabel('services_payment'), value: curWidget.services_payment})
  ...
  const RevenueForecastDetail = quantity.map((el, i) => {
    return (
      <div className='revenue-forecast__dropdown' key={i}>{el.name} - {el.value}</div>
    )
  })
      ...
      <div className='revenue-forecast__panel-value'>
        <div className='revenue-forecast__panel-value__wrapper'>
          <div className='revenue-forecast__panel-value__switcher'>
            <input type='checkbox' checked={isVisible} id='revenue-forecast-switcher' onChange={(event) => setIsVisible(event.target.checked)} />
