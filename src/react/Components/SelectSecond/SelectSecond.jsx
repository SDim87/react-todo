  ...
  const getListOptions = () => {
    if (options[0]) {
      return options.map(el => {
        return (
          <li className='select__option' key={el} onClick={() => {
            setDataInput(el)
            setOpen(false)
          }}>
        ...
        <div className={`select__box-wrapper ${options[0] ? '' : 'with-no-icons'}`}>
          <div className='select__box'>
            <div className={`select__input ${open && options[0] ? 'is-opened' : ''}`}>
              <input className='select__search' type='search' name='search' id='' readOnly autoComplete='false' placeholder={dataInput} onClick={() => setOpen(!open)}/>
            </div>
          </div>
