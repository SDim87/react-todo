...
const ConversionGeneral = ({ text, }) => {
  const [requestResult, setRequestResult] = useState('')

  useEffect(() => {
    widgetContentFunction('widget_convertion_general').then(res => setRequestResult(res))
  }, [])

  if (requestResult) {
    const { content, } = requestResult.data.widget_convertion_general
